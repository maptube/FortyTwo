...
function TubeDataLayer() {
	this.bbox = null;						//bounding box for data which is set when it is loaded - this comes from the collada line network only
	...
	this.mesh_tubeLines = null;				//this is the mesh showing the position of the lines
	
	//tubeAgentsHelper : Object.create(AgentsHelper),	//trying to make an abstraction of the agent animation to make the buses work
	this.tubeAgentsHelper = new AgentsHelper2(),

	//constants defining where some of the data comes from
	this.urlTubeNetworkJSON = 'realtime/tube-network.json';
	this.urlStationCodesCSV = 'data/station-codes.csv';
	this.urlTubeLinesDAE = 'data/TubeLines_thin_ECEF.DAE';
	this.urlTubeLinesOBJ = 'data/TubeSplines_ECEF.obj';
	this.urlRealTimeTubes = 'http://loggerhead.casa.ucl.ac.uk/api.svc/f/trackernet?pattern=trackernet_*.csv';
}

/**
* Second animate function using the helper
*/
TubeDataLayer.prototype.animate = function (earth) {
...
*/
TubeDataLayer.prototype.lineCodeToColour = function(c) {
...
*/
TubeDataLayer.prototype.load = function (earth,ondataloaded) {
	this.loadnetwork();
...
*
*/
TubeDataLayer.prototype.loadnetwork = function () {
...
*/
TubeDataLayer.prototype.loadtubelinesmesh = function(earth,ondataloaded) {
...
*/
TubeDataLayer.prototype.loadtubes = function (earth) {
...
function BusDataLayer() {

	this.bbox = null;
	this.busParent = null; //object that is the parent to all the bus cubes
	//busAgentsHelper : Object.create(AgentsHelper), //used to animate the buses
	this.busAgentsHelper = new AgentsHelper2();
	
	//urlRealTimeBuses : 'realtime/countdown_20130418_135400.csv',
	this.urlRealTimeBuses = 'http://loggerhead.casa.ucl.ac.uk/api.svc/f/countdown?pattern=countdown_*.csv';
	//urlStopCodesJSON : 'realtime/countdown/stops.json',
	this.urlStopCodesJSON = 'realtime/countdown/stops_json.csv'; //have to do this as it's not valid json
...
BusDataLayer.prototype.load = function (earth,ondataloaded) {
	this.loadNetwork();
...
*/
BusDataLayer.prototype.loadNetwork = function () {
	//had to use closures to get the object data to the xmlhttp request complete event
	//$.getJSON(this.urlTubeNetworkJSON,
	//	function (inner_parent) {
	//		return function(json) {
