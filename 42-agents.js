...
function AgentsHelper2() {
	this.dataTime = new Date(); //validity time of the data
	this.agents = new Array(); //list of animrecs
	this.vertexList = new Array(); //list of vertex names and xyz positions
	this.odNetwork = new Object(); //origin destination network - loaded from JSON asynchronously
}

AgentsHelper2.prototype.setDataTime = function (dt) {
	this.dataTime.setTime(dt.getTime()); //not exactly intuitive, but sets date AND time (also recommended to use valueOf)
};

AgentsHelper2.prototype.setODNetwork = function (odlinks) {
	//Need to do it this way to keep the odNetwork within the prototype AgentsHelper object
	//JS object merge - TODO: check you still need to do this using prototypes
	for (var attrname in odlinks) { this.odNetwork[attrname] = odlinks[attrname]; }
};

AgentsHelper2.prototype.animate = function (nowTime) {
//var now = new Date();
...
AgentsHelper2.prototype.simpleAnimate = function(nowTime) {
	var elapsedSecs = (nowTime-this.dataTime)/1000; //between data time and now
	for (var i in this.agents) { //i is actually the name of the agent, not an index number
		var anim_rec = this.agents[i];
		if (anim_rec!=null) {
